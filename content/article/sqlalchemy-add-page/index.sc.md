...
---
<!--more-->
有过这样一个需求，需要返回的RESTful API里，需要通过后端实现不同字段的排序或筛选或者兼而有之，以及分页逻辑，ORM引擎为SQLAlchemy，这个确实做起来比较坑，大量的代码非常不优雅，秉着不重复造轮子的做法，通过阅读Flask-SQLAlchemy的源码，为SQLALchemy添加分页逻辑。  
